...
// - decreases by 1 whenever one cloned Rc is dropped out of the scope
// - When an Rc's reference count becomes zero, both the Rc and the value are all dropped
// - which means there are no remaining owners
// - Cloning an Rc never performs a deep copy but
// - creates just another pointer to the wrapped value, and
